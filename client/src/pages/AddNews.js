...
function AddNews() {
  ...
  const user = JSON.parse(localStorage.getItem('adnews-user'))
  console.log(user);
  ...
  return (
    <Layout>
        ...
        <div className='px-5 pt-5'>
          <input value={title} onChange={(e)=>setTitle(e.target.value)} type="text" className='border-2 h-10 w-full border-gray-300 px-5 mb-2' placeholder='Title' />
          <textarea value={description} onChange={(e)=>setDescription(e.target.value)} className='border-2 w-full border-gray-300 my-2 px-5' rows="4" placeholder='Description' ></textarea>
        </div>
