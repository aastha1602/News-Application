...
function HomePage() {
  ...
  return (
    <Layout>
        ...
        <input value={searchText} onChange={(e)=>setSearchText(e.target.value)} type="text" className='border-2 h-10 w-full border-gray-300 px-5' placeholder='Search news' />
        </div>
        {newsItems.length > 0 && (
          <div className='grid grid-cols-2 sm:grid-cols-1 gap-5 mx-20 sm:mx-5 my-5'>
            {/* <h1>{newsItems.length}</h1> */}
            {newsItems.filter((item)=>item.title.toLowerCase().includes(searchText.toLowerCase())).map((item)=>{
              return <div className='shadow-md p-3 border cursor-pointer' onClick={()=>navigate(`/newsdesc/${item._id}`)}>
                ...
                <div className='flex justify-end flex-col items-end'>
                      <span className='text-gray-500 text-sm'>By : {item.postedBy.email}</span>
