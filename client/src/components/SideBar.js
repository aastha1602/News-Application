...
function SideBar({showSideBar}) {
  ...
  const logout=()=>{
    localStorage.removeItem('adnews-user')
    navigate('/')
  }

  return (
    // <div className={' bg-primary h-screen flex flex-col overflow-hidden ${showSideBar ? 'w-60' : 'w-0' } '}>
    <div
  ...
  >
      <div className='flex justify-center flex-col ml-10'>
        <h1 className='text-2xl font-bold mt-6 text-gray-400'>AD</h1>
      ...
      {menuItems.map((item) => {
          return  (
            item.title!=='Logout' ? <Link
              to={`${item.path}`}
              className=
              {`  pl-10 py-5 text-gray-400 hover:bg-gray-50 hover:text-gray-700 text-sm
                  ${
